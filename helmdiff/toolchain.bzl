...
def _helmdiff_toolchain_impl(ctx):
    ...
    # Make the $(HELMDIFF_BIN) variable available in places like genrules.
    # See https://docs.bazel.build/versions/main/be/make-variables.html#custom_variables
    template_variables = platform_common.TemplateVariableInfo({
        "HELMDIFF_BIN": target_tool_path,
        # Variable to use with disabled legacy_external_runfiles
        "HELMDIFF_RUNFILES_BIN": target_tool_path.replace("external/", "../", 1),
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    helmdiffinfo = HelmdiffInfo(
...
helmdiff_toolchain = rule(
    implementation = _helmdiff_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a helmdiff compiler/runtime toolchain.
